...
export async function getLieuTouristique(position, distance) {
  ...
  await fetch(
    `https://data.culture.gouv.fr/api/records/1.0/search/?dataset=base-des-lieux-et-des-equipements-culturels&q=&lang=fr&rows=5&geofilter.distance=${position[1]}%2C${position[0]}%2C${distance}`,
    { method: "GET" }
  )
    ...
    .then((response) => {
      ...
      for (let i = 0; i < lieux.length; i++) {
        lieux_all.push({
          coordinate: lieux[i].geometry.coordinates,
          nom: lieux[i].fields.nom,
        });
      }
    });
  return lieux_all;
...
export async function getDistanceEstime(position, lieux_selectionne, type) {
  ...
  const accessToken =
    "pk.eyJ1IjoiY3ZuZHNoIiwiYSI6ImNsZ3V1ZzQxMzAxanMzbG11Z2E0cWJ0bmgifQ.7BRdbaWmQytXDZ8AE4CIaA";
  let query;
  let arret_point = "";
  for (let i = 0; i < lieux_selectionne.length; i++) {
    arret_point =
      arret_point +
      lieux_selectionne[i].coordinate[0] +
      "," +
      lieux_selectionne[i].coordinate[1] +
      ";";
  }
  if (type === "bike") {
    // pour velo
...
export async function getRoute(position, distance, type) {
  ...
  return {
    distance: distaceEstime,
    temps: tempsEstime,
    route: parcoursCree,
    lieux_tour: lieux_selectionne,
  };
}
