...
module.exports = {
  async newPlace (req, res) {
    ...
    // #swagger.reponses[201] = { description: 'New place successfully added.'}
    return res
      .status(201)
  ...
  async getPlaceByCoords (req, res) {
    ...
    console.log(req.params.latitude, typeof (req.params.latitude))
    const data = await placesModel.findOne({
    ...
    // #swagger.reponses[201] = { description: 'Place successfully found.'}
    return res
      .status(200)
      .send({ status: status.OK, message: 'Lieu récupéré', data })
  }
}
