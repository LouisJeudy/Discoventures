...
export default function GeneratedParcourAvecMap({route,navigation}){
  ...
  const toggleSwitch = () => setvisiblePublic(previousState => !previousState);
  ...
  async function EnregistrerParcours(){
    ...
    for(let indice = 0; indice < lieux.length; indice++){
      ...
      await fetch(`${BACKEND}/places`,{
            ...
            body: JSON.stringify({data:JSON.stringify({title:lieux[indice].nom, description:descrip[indice],longitude:lieux[indice].coordinate[0], latitude:lieux[indice].coordinate[1]})})
        }).then(response => response.json())
        .then(response =>{
    ...
    await fetch(`${BACKEND}/routes`,{
      ...
      body:  JSON.stringify({data:JSON.stringify({title:name, coordinates:{data:{ latitude: latitude_, longitude:longitude_}}
        ,estimatedDistance:distance
        ,estimatedTime:temps,activityType:activity,isPrivate:!visiblePublic,places:{ids:id_lieux}})})
    }).then(response => response.json())
    .then(response =>{
      if(response.status!= 201){ 
        setErrormsg(response.message)
