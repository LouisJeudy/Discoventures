...
import MapCard from "../../../components/uikit/MapCard";

const BACKEND = "https://discoventures.osc-fr1.scalingo.io";

// Page pour que les utilisateurs découvrent tous les parcours publiques partagées par d'autres utilisateurs
export default function Decouvertes(props) {
  ...
  async function VisualiserParcours(route) {
    await fetch(`${BACKEND}/routes/${route.id}`, {
      ...
      .then((response) => {
        if (response.status == 200) {
          ...
          for (let i = 0; i < response.data.places.length; i++) {
            lesLieux.push({
              coordinate: [
                response.data.places[i].longitude,
                response.data.places[i].latitude,
              ],
              nom: response.data.places[i].title,
            });
            descrip.push(response.data.places[i].description);
          }
          // Redirection vers le parcours en détails
      ...
      .catch((error) => alert("Server error visualiser parcours :" + error));
  }
  return (
        ...
        {routeLoaded ? (
          routes.map((route) => {
            ...
            return (
              <MapCard
                key={route.id}
                nativeID={"decouvertesMapCard" + route.id}
                onPress={async () => VisualiserParcours(route)}
                title={route.title}
                activityType={route.activityType}
                distance={Math.round(route.estimatedDistance / 1000)}
                estimatedTime={route.estimatedTime}
                isPrivate={route.isPrivate}
                nbVoters={route.nbVoters}
                score={route.score}
